...
export default function App() {
  ...
  // GitHub Repository QA Audit states
  const [isGitHubAuditModalOpen, setIsGitHubAuditModalOpen] = useState<boolean>(false);
  ...
  const [selectedAuditRepoUrl, setSelectedAuditRepoUrl] = useState<string>("facebook/react");
  const [isAuditingRepo, setIsAuditingRepo] = useState<boolean>(false);
  const [githubAuditReport, setGithubAuditReport] = useState<any | null>(null);
  const [githubAuditError, setGithubAuditError] = useState<string | null>(null);
  const [badgeCopiedFormat, setBadgeCopiedFormat] = useState<string | null>(null);

  // Fetch GitHub repos when modal opens
  React.useEffect(() => {
    if (isGitHubAuditModalOpen && authState.authenticated) {
      setIsLoadingRepos(true);
      const apiHost = window.location.port === "1420" ? "http://127.0.0.1:4567" : "";
      fetch(`${apiHost}/api/github/repos`)
        .then((res) => res.json())
        .then((data) => {
          if (data && Array.isArray(data.repos)) {
            setGithubRepos(data.repos);
            if (data.repos.length > 0 && selectedAuditRepoUrl === "facebook/react") {
              setSelectedAuditRepoUrl(data.repos[0].fullName);
            }
          }
        })
        .catch(() => {})
  ...
  }, [isGitHubAuditModalOpen, authState.authenticated]);

  async function executeGitHubAudit(repoTarget?: string) {
    const target = repoTarget || selectedAuditRepoUrl;
    if (!target.trim()) return;
    setIsAuditingRepo(true);
    setGithubAuditError(null);
    try {
      const apiHost = window.location.port === "1420" ? "http://127.0.0.1:4567" : "";
      const res = await fetch(`${apiHost}/api/github`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ repoUrl: target.trim() }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "GitHub QA Analysis failed");
      setGithubAuditReport(data);
      setStatusToast(`Audited ${data.repo?.fullName || target}: QA Score ${data.qaScore?.totalScore ?? 100}/100`);
      setTimeout(() => setStatusToast(null), 4000);
    } catch (err: any) {
      setGithubAuditError(err.message || "Failed to audit repository.");
    } finally {
      setIsAuditingRepo(false);
    }
  }

  function copyBadgeCode(code: string, format: string) {
    navigator.clipboard.writeText(code);
    setBadgeCopiedFormat(format);
    setTimeout(() => setBadgeCopiedFormat(null), 2500);
  }
      ...
      fetch(`${apiHost}/api/projects`)
        ...
        .then((data) => {
          if (data && Array.isArray(data.projects) && data.projects.length > 0) {
            ...
            const current = data.projects.find((p: any) => p.isCurrent);
            if (current && (!targetFolderPath || targetFolderPath.endsWith("\\dist") || targetFolderPath.endsWith("/dist"))) {
              setTargetFolderPath(current.path);
            }
          }
        })
        .catch(() => {});
  ...
  async function handleBrowseFolder() {
    ...
    try {
      if (window && (window as any).__TAURI_INTERNALS__) {
        ...
        // Local GUI server mode: open native OS folder dialog
        const apiHost = window.location.port === "1420" ? "http://127.0.0.1:4567" : "";
        const res = await fetch(`${apiHost}/api/dialog/folder`, { method: "POST" });
        if (res.ok) {
          const data = await res.json();
          if (data?.path) {
            setTargetFolderPath(data.path);
            return;
          }
        }
...
✅ Safe to update ${selectedNode.name}!`,
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-[#070b14] text-slate-100 overflow-hidden font-sans select-none relative">
      ...
      <header className="h-12 border-b border-slate-800/80 bg-slate-950/90 px-4 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => {
              if (!authState.authenticated) {
                setStatusToast("GitHub login required before auditing repositories.");
                setIsLoginOpen(true);
                return;
              }
              setIsGitHubAuditModalOpen(true);
            }}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-xs font-bold text-white transition shadow-lg shadow-blue-500/20 cursor-pointer"
            title="Audit GitHub Repositories & generate official Developer Program badge"
          >
            <Globe className="w-3.5 h-3.5 text-cyan-200" />
            <span>GitHub QA Audit</span>
          </button>
      ...
      {/* GitHub Repository QA Audit Modal */}
      {isGitHubAuditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-4xl max-h-[90vh] rounded-2xl bg-slate-900 border border-blue-500/30 shadow-[0_0_60px_rgba(59,130,246,0.15)] flex flex-col overflow-hidden">
            {/* Header */}
            <div className="px-6 py-4 border-b border-slate-800 bg-slate-950/80 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-100 flex items-center gap-2">
                    <span>GitHub Repository QA Audit</span>
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-500/10 text-blue-400 border border-blue-500/20">
                      Developer Program
                    </span>
                  </h3>
                  <p className="text-xs text-slate-400 mt-0.5">
                    Audit code health, dependencies & generate live Developer Program badge
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setIsGitHubAuditModalOpen(false)}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-400">
                    Format: <code className="text-blue-300">owner/repo</code> or full GitHub URL
                  </span>
                </div>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={selectedAuditRepoUrl}
                    onChange={(e) => setSelectedAuditRepoUrl(e.target.value)}
                    placeholder="e.g. facebook/react or owner/repo"
                    className="flex-1 rounded-xl bg-black/60 border border-slate-700/80 px-4 py-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 font-mono transition"
                  />
                  <button
                    type="button"
                    onClick={() => executeGitHubAudit()}
                    disabled={isAuditingRepo || !selectedAuditRepoUrl.trim()}
                    className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-50 text-xs font-bold text-white transition flex items-center gap-2 shadow-lg shadow-blue-500/25 cursor-pointer shrink-0"
                  >
                    {isAuditingRepo ? (
                      <>
                        <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                        <span>Auditing QA...</span>
                      </>
                    ) : (
                      <>
                ...
                {githubAuditError && (
                  <div className="p-3 rounded-lg bg-rose-950/80 border border-rose-500/40 text-rose-200 text-xs flex items-center gap-2">
                    ...
                    <span>{githubAuditError}</span>
                  </div>
                )}
              </div>

              {/* My Repositories Quick Selector */}
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                  ...
                  {githubRepos.length > 0 && (
                    <input
                      ...
                      className="w-48 rounded-lg bg-black/40 border border-slate-800 px-2.5 py-1 text-[11px] text-slate-300 placeholder-slate-500 focus:outline-none focus:border-blue-500"
                    />
                  )}
                </div>
                ...
                {isLoadingRepos ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                    {githubRepos
                      .filter((r) =>
                        repoSearchQuery ? r.fullName.toLowerCase().includes(repoSearchQuery.toLowerCase()) : true
                      )
                      .slice(0, 30)
                      .map((repo) => (
                        <div
                          key={repo.id}
                          onClick={() => {
                            setSelectedAuditRepoUrl(repo.fullName);
                            void executeGitHubAudit(repo.fullName);
                          }}
                          className={`p-2.5 rounded-xl border text-left transition cursor-pointer flex flex-col justify-between gap-1.5 ${
                            selectedAuditRepoUrl === repo.fullName
                  ...
                  <div className="py-4 text-center text-slate-500 text-xs">
                    No repositories found or permission not granted. You can type any repository above.
                  </div>
                )}
              </div>
              ...
              {githubAuditReport && (
                <div className="space-y-4 pt-4 border-t border-slate-800 animate-in fade-in duration-300">
                  {/* Top Score Banner */}
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-2xl bg-gradient-to-r from-slate-950 to-slate-900 border border-blue-500/30 shadow-lg">
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="text-base font-bold text-slate-100">
                          {githubAuditReport.repo?.fullName}
                        </h4>
                        <a
                          href={githubAuditReport.repo?.htmlUrl}
                          target="_blank"
                      ...
                      <p className="text-xs text-slate-400 mt-1 max-w-lg">
                        {githubAuditReport.repo?.description || "No description provided"}
                      </p>
                    </div>
                    <div className="text-right shrink-0">
                      <span className="text-[11px] text-slate-400 block font-medium">QA Health Score</span>
                      <span className="text-2xl font-black text-emerald-400">
                        {githubAuditReport.qaScore?.totalScore ?? 100}/100
                      </span>
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 block font-bold mt-1">
                        Grade {githubAuditReport.qaScore?.grade || "A+"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800/80">
                      ...
                      <span className="text-sm font-bold text-yellow-400 mt-1 block">
                        ⭐ {githubAuditReport.repo?.stars?.toLocaleString() || 0}
                      </span>
                    </div>
                    <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800/80">
                      <span className="text-[10px] text-slate-400 block">PR Merge Rate</span>
                      <span className="text-sm font-bold text-blue-400 mt-1 block">
                        {githubAuditReport.pullRequests?.mergeRatePercent ?? 100}%
                      </span>
                    </div>
                    <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800/80">
                      <span className="text-[10px] text-slate-400 block">Direct Dependencies</span>
                      <span className="text-sm font-bold text-emerald-400 mt-1 block">
                        {githubAuditReport.dependencies?.totalDirectDependencies || 0} Packages
                      </span>
                    </div>
                    <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800/80">
                      <span className="text-[10px] text-slate-400 block">Last Commit</span>
                      <span className="text-sm font-bold text-purple-400 mt-1 block">
                        {githubAuditReport.commits?.lastCommitDate
                          ? githubAuditReport.commits.lastCommitDate.slice(0, 10)
                          : "Recent"}
                      </span>
                    </div>
                  </div>

                  {/* Official Developer Program Badge Generator */}
                  <div className="p-5 rounded-2xl bg-gradient-to-b from-slate-950 to-black border border-blue-500/20 space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h5 className="text-xs font-bold text-slate-200 flex items-center gap-2">
                          <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                          <span>Official Developer Program Badge</span>
                        </h5>
                        <p className="text-[11px] text-slate-400 mt-0.5">
                          Add this live badge to your repository README to show verified code QA health
                        </p>
                      </div>
                      <div className="px-2 py-1 rounded bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-mono text-[10px] font-bold">
                        VERIFIED
                      </div>
                    </div>

                    {/* Live Badge Preview */}
                    <div className="p-4 rounded-xl bg-black/80 border border-slate-800 flex items-center justify-center">
                      <img
                        src={`https://img.shields.io/badge/BreakGuard%20QA-Score%20${githubAuditReport.qaScore?.totalScore ?? 100}%2F100-10b981?style=for-the-badge&logo=shield`}
                        alt="BreakGuard QA Badge"
                        className="h-7"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <button
                        ...
                        onClick={() =>
                          copyBadgeCode(
                            `[![BreakGuard QA](https://img.shields.io/badge/BreakGuard%20QA-Score%20${githubAuditReport.qaScore?.totalScore ?? 100}%2F100-10b981?style=for-the-badge&logo=shield)](https://github.com/${githubAuditReport.repo?.fullName})`,
                            "markdown"
                          )
                        }
                        className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-md transition cursor-pointer"
                      >
                        {badgeCopiedFormat === "markdown" ? (
                          <Check className="w-3.5 h-3.5 text-emerald-300" />
                        ) : (
                          <Copy className="w-3.5 h-3.5" />
                        )}
                        <span>
                          {badgeCopiedFormat === "markdown" ? "Copied Markdown!" : "Copy Markdown Badge"}
                        </span>
                      </button>
                      ...
                      <button
                        ...
                        onClick={() =>
                          copyBadgeCode(
                            `<a href="https://github.com/${githubAuditReport.repo?.fullName}"><img src="https://img.shields.io/badge/BreakGuard%20QA-Score%20${githubAuditReport.qaScore?.totalScore ?? 100}%2F100-10b981?style=for-the-badge&logo=shield" alt="BreakGuard QA" /></a>`,
                            "html"
                          )
                        }
                        className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold transition cursor-pointer"
                      >
                        {badgeCopiedFormat === "html" ? (
                          <Check className="w-3.5 h-3.5 text-emerald-300" />
                        ) : (
                          <Code2 className="w-3.5 h-3.5" />
                        )}
                        <span>{badgeCopiedFormat === "html" ? "Copied HTML!" : "Copy HTML Badge"}</span>
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div className="px-6 py-3.5 border-t border-slate-800 bg-slate-950/90 flex items-center justify-between text-xs">
              <span className="text-slate-400 text-[11px]">
                Audits real-time commit velocity, PR merge rate, and AST dependency risks.
              </span>
              <button
                type="button"
                onClick={() => setIsGitHubAuditModalOpen(false)}
